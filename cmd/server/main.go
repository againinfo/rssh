...
func printHelp() {
	...
	fmt.Println("\t--ui [addr]\t\tEnable web UI (defaults to 127.0.0.1:8080)")
	fmt.Println("\t--ui-user\t\tBasic auth username for UI (or RSSH_UI_USER)")
	fmt.Println("\t--ui-pass\t\tBasic auth password for UI (or RSSH_UI_PASS)")
	fmt.Println("\t--ui-no-sse\t\tDisable live updates (SSE) in UI")
...
func main() {
	...
	if options.IsSet("ui") {
		...
		if v, err := options.GetArgString("ui"); err == nil && strings.TrimSpace(v) != "" {
			uiAddr = strings.TrimSpace(v)
		}
	}
	if uiAddr == "" {
		if v := strings.TrimSpace(os.Getenv("RSSH_UI_ADDR")); v != "" {
			uiAddr = v
		}
	}

	uiUser := ""
	if v, err := options.GetArgString("ui-user"); err == nil {
		uiUser = strings.TrimSpace(v)
	} else {
		uiUser = strings.TrimSpace(os.Getenv("RSSH_UI_USER"))
	}

	uiPass := ""
	if v, err := options.GetArgString("ui-pass"); err == nil {
		uiPass = v
	} else {
		uiPass = os.Getenv("RSSH_UI_PASS")
	}

	uiSSE := !options.IsSet("ui-no-sse")
	if v, ok := os.LookupEnv("RSSH_UI_SSE"); ok {
	...
	server.Run(listenAddress, dataDir, connectBackAddress, autogeneratedConnectBack, tlscert, tlskey, insecure, enabledDownloads, tls, openproxy, timeout, uiAddr, uiUser, uiPass, uiSSE)
}
