...
func parseSingleArg(line string, startPos int) (arg Argument, endPos int) {
	...
	for i := startPos; i < len(line); i++ {
		...
		// End of argument (space delimiter) only when not in quotes.
		if !inSingleQuote && !inDoubleQuote && c == ' ' {
			return
		}

		// Single quotes: preserve everything literally, except allow \' to be a literal quote
		// without ending the quoted section (and preserve the backslash as written).
		if inSingleQuote {
		...
		// Double quotes: allow escaping of " and ' via backslash (remove the backslash).
		// Keep backslash for other characters (e.g. \" -> ", \  -> \ ).
		if inDoubleQuote {
			if c == '\\' && i+1 < len(line) {
				next := line[i+1]
				if next == '"' || next == '\'' {
		...
		if c == '\\' && i+1 < len(line) {
			next := line[i+1]
			if next == ' ' {
				// Allow escaping space outside quotes (drop the backslash).
				sb.WriteByte(' ')
				i++
				arg.end = i
				continue
			...
			// Preserve the backslash, but treat the next char as literal (do not start/end quotes).
			sb.WriteByte('\\')
			sb.WriteByte(next)
			i++
			arg.end = i
