...
func StartSSHServer(sshListener net.Listener, privateKey ssh.Signer, insecure, openproxy bool, dataDir string, timeout int) {
	...
	config := &ssh.ServerConfig{
		...
		PublicKeyCallback: func(conn ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			return nil, fmt.Errorf("not authorized %q (client key sha256=%s), potentially you might want to enable --insecure mode", conn.User(), internal.FingerprintSHA256Hex(key))
		},
	}
...
func acceptConn(c net.Conn, config *ssh.ServerConfig, timeout int, dataDir string) {
	...
	if err != nil {
		// Include remote address so auth/hostkey issues can be diagnosed (common with Windows clients running detached).
		log.Printf("Failed to handshake from %s (%s)", c.RemoteAddr().String(), err.Error())
		return
	}
	...
	if timeout > 0 {
		...
		go func() {
			for {
				interval := timeout
				payload := timeout
				if v := strings.TrimSpace(sshConn.Permissions.Extensions["server-timeout"]); v != "" {
				...
				// Don't require a reply: waiting for a response under a net.Conn deadline can cause
				// periodic disconnect loops when the reply is delayed or dropped.
				if _, _, err := sshConn.SendRequest("keepalive-rssh@golang.org", false, []byte(fmt.Sprintf("%d", payload))); err != nil {
					clientLog.Info("Failed to send keepalive, assuming client has disconnected")
					sshConn.Close()
					return
				}
				// Sleep based on the effective interval we told the client to use.
				// If payload==0 (disable timeouts), fall back to the server's default interval.
				if interval <= 0 {
					interval = timeout
				}
	...
	case "client":
		...
		fp := strings.TrimSpace(sshConn.Permissions.Extensions["pubkey-fp"])

		// Apply persisted comm settings (best-effort).
		if fp != "" {
			if s, ok, err := data.GetClientCommSettings(fp); err == nil && ok {
				sshConn.Permissions.Extensions["server-timeout"] = fmt.Sprintf("%d", s.ServerTimeoutSeconds)
				_, _, _ = sshConn.SendRequest("keepalive-rssh@golang.org", false, []byte(fmt.Sprintf("%d", s.ServerTimeoutSeconds)))
				_, _, _ = sshConn.SendRequest("client-heartbeat@rssh", false, []byte(fmt.Sprintf("%d", s.ClientHeartbeatSec)))
				b, _ := json.Marshal(map[string]any{"window": s.SleepWindow, "check": s.SleepCheckSec})
				_, _, _ = sshConn.SendRequest("sleep-window@rssh", false, b)
				b2, _ := json.Marshal(map[string]any{"until": s.SleepUntil})
				_, _, _ = sshConn.SendRequest("sleep-until@rssh", false, b2)
			}
		}
