...
func Run(addr, dataDir, connectBackAddress string, autogeneratedConnectBack bool, TLSCertPath, TLSKeyPath string, insecure, enabledDownloads, enabletTLS, openproxy bool, timeout int, uiAddr, uiUser, uiPass string, uiSSE bool) {
	...
	if uiAddr != "" {
		if uiUser == "" || uiPass == "" {
			log.Fatal("ui enabled but basic auth user/pass missing (set --ui-user/--ui-pass or RSSH_UI_USER/RSSH_UI_PASS)")
		}

		go func() {
			if err := ui.Start(ui.Config{
				Addr:      uiAddr,
				Username:  uiUser,
				Password:  uiPass,
				EnableSSE: uiSSE,
			}); err != nil {
				log.Println("ui server stopped:", err)
			}
		}()
	}
